	...
	render() {
	    return (
	      <div className="choiceButton col-md-12" style={{padding: '10px'}}>
	        <button onClick={() => this.props.click()} disabled={this.props.disabled} className="btn btn-primary" style={{ width: '100%' }}>{this.props.title}</button>
	      </div>
	    );
	}
}
...
class Game extends React.Component {
  ...
  loadQuestion() {
    ...
    setTimeout(() => {
      this.questionLoaded({id: 12, question: 'What color is it?', choices: {a: 'red', b: 'green', c: 'blue', d: 'yellow',}})
    }, 1000);
  }
  ...
  selectChoice(i) {
    ...
    setTimeout(a => { this.resultLoaded({ result: i === 'b' }); }, 1000);
  }
  ...
  render() {
    ...
    let buttons = this.state.choices.map(item => {
      return (<ChoiceButton key={item.value} value={item.value} title={item.title} click={item.click} disabled={choicesDisabled} />);
    });
